...
/* eslint-disable */
var store = []; // Store keep tracks of userId and its corresponding socket.

var SocketStore =
/*#__PURE__*/
function () {
  function SocketStore() {
    _classCallCheck(this, SocketStore);
  }

  _createClass(SocketStore, null, [{
    key: "addByUserId",
     ...
     */
    value: function addByUserId(userId, socketObj) {
    ...
    key: "removeByUserId",
    value: function removeByUserId(userId, socketObj) {
      if (userId === null || userId === undefined || socketObj === null || socketObj === undefined) {
        ...
        for (var i = 0; i < store.length; i++) {
          if (store[i].id === userId) {
            var socketObjectIndex = store[i].socket.indexOf(socketObj);
    ...
    key: "emitToAll",
    value: function emitToAll(eventName, payload) {
      if (store.length === 0) {
        return returnFunc(false, 'socket store is empty', '');
      } else {
        for (var i = 0; i < store.length; i++) {
    ...
    value: function display() {
      for (var i = 0; i < store.length; i++) {
        // console.log('--------->\nuserId', store[i].id, '\nsocketId\n');
        for (var j = 0; j < store[i].socket.length; j++) {
