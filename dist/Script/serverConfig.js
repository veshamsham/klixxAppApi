...
var databaseURL = "mongodb://localhost:27017/".concat(databaseName); //const databaseURL = `mongodb+srv://nitprise:Rishabh2019@cluster0-sclfx.mongodb.net/test?retryWrites=true&w=majority`;
...
var ServerConfig = mongoose.model("ServerConfig", ServerConfigSchema); // Async series method to make sure asynchronous calls below run sequentially
...
function (callback) {
  ...
  serverConfigs.push(eval(value)); // console.log(eval(value));

  /*eslint-disable */

  console.log("Populating database with %s serverConfigs", serverConfigs.length); // Use 'async.eachSeries' to loop through the 'users' array to make
